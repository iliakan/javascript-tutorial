...
Метод `elem.closest(css)` ищет ближайший элемент выше по иерархии DOM, подходящий под CSS-селектор `css`,  или null, если таковых элементов вообще нет. Сам элемент тоже включается в поиск.
