...
Looking ahead let's just note that `"use strict"` can be put at the start of a function (with minor limitations) instead of the whole script. Then strict mode is enabled in that function only. But usually people use it for the whole script.
```
