...
```
Отличие `parseInt` и `parseFloat`также в том, что `parseInt` может принимать вторым аргументом целое число в диапазоне между 2 и 36, представляющее собой основание системы счисления числовой строки. То есть, для установки десятичной системы счисления надо передать 10.

```js
//+ run
alert(parseInt('12px', 2)) //  1, двоичная система
alert(parseInt('12px', 10)) // 12, десятичная
```
Что происходит, если основание системы счисления имеет значение undefined (неопределено) или равно 0 (или не указано), подробнее можно прочитать здесь [особенности parseInt](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/parseInt;)
